...
function tokenErrorMetadata(payload: unknown) {
  ...
  return {
    ...
    description: typeof row.error_description === "string" ? row.error_description.slice(0, 160) : null,
  };
}
...
export async function GET(request: Request) {
  ...
  if (!code || !state || !nuvemshopOAuthConfigured()) {
    return appRedirect(request, "error");
  }

  const tokenResult = await exchangeNuvemshopCode(code);
  if (!tokenResult.ok) {
  ...
  await upsertNuvemshopCredential({
    companyId: context.company.id,
    userId: context.user.id,
    token,
  });
