            ...
            {aSeparar.concat(aEmbalar).slice(0, 5).map((o) => {
              ...
              return (
                <div className="lrow om-row-click" key={o.id} onClick={() => go("pedidos", { open: o.id })}>
                  <div className={cn("chip", `chip--${st.tone}`)}><Icon name="pedidos" size={16} /></div>
                  <div className="lrow-main">
                    <div className="lrow-title">{o.num} · {o.customerName}</div>
                    <div className="lrow-sub">{qty} {qty > 1 ? "itens" : "item"} · {CHANNELS[o.channel]} · {o.city}</div>
                  </div>
              ...
              {abaixoMin.slice(0, 4).map((i) => (
                <div className="lrow om-row-click" key={i.id} onClick={() => go("itens", { open: i.code })}>
                  ...
                  <div style={{ textAlign: "right" }}>
                    <div style={{ fontWeight: 650, fontSize: 13.5 }} className="om-text--bad">{num(i.available)} {i.unit}</div>
