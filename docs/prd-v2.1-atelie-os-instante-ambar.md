# PRD v2.1 — Ateliê OS: Backoffice Artesanal White Label

> **Versão 2.1 — otimizada para agente de IA / Codex**  
...
Esta versão adiciona duas frentes estruturais que devem ser consideradas desde a fundação do produto:

1. **Branding white label configurável**: página de configurações para logotipo, identidade visual, temas de cores, presets, tokens via JSON, preview e aplicação segura no app.
2. **Kanban e fluxos configuráveis**: produção, pedidos e outras entidades operacionais não devem depender de nomes fixos como “Em cura”. O usuário deve poder criar/editar etapas, cores, ordem, checklists e automações associadas a chaves técnicas seguras.

Esses requisitos são importantes porque o primeiro uso será a Instante Âmbar, mas o sistema deve nascer preparado para outros tipos de ateliês e pequenos fabricantes artesanais.
...
## 7.30 Branding white label — logo, temas e cores configuráveis

### Objetivo

Permitir que cada empresa personalize a aparência do sistema sem alterar código. A Instante Âmbar terá uma identidade inicial própria, mas o sistema deve nascer preparado para uso white label por outros negócios artesanais.

A personalização visual deve cobrir:

- logotipo;
- ícone/símbolo reduzido;
- nome público da empresa;
- tema de cores;
- presets de combinação de cores;
- tokens visuais salvos em JSON;
- preview antes de aplicar;
- restauração para tema padrão.

### Princípio de design técnico

A interface deve ser construída com tokens, não com cores fixas espalhadas pelo código.

O sistema deve gerar CSS variables a partir das configurações da empresa. Componentes shadcn/Tailwind devem consumir tokens semânticos, como:
...
- `owner`: pode alterar tudo;
- `admin`: pode alterar se a empresa permitir;
- demais papéis: somente leitura ou sem acesso.
...
- nome interno da empresa;
- nome de exibição;
- subtítulo opcional;
- logotipo principal;
- logotipo reduzido/ícone;
- favicon/app icon futuro;
- texto alternativo do logo;
- opção “usar nome textual se logo não estiver disponível”.
...
- gerar preview;
- preservar versão anterior até salvar;
- permitir remover logo e voltar ao texto.
...
- preset selecionado;
- modo padrão: claro, escuro ou automático;
- JSON completo do tema;
- botão “validar tema”;
- botão “pré-visualizar”; 
- botão “salvar”; 
- botão “restaurar padrão”.

#### 3. Editor simples de cores
...
O editor simples deve atualizar o JSON automaticamente.

#### 4. Editor avançado JSON

Permitir editar diretamente os tokens do tema em JSON.

O JSON deve aceitar múltiplas combinações:
...
### Presets iniciais obrigatórios

O sistema deve vir com presets seedados:

1. **Instante Âmbar** — tons creme, âmbar, marrom suave, sofisticado e acolhedor.
2. **Neutro claro** — cinza/azul discreto para uso genérico.
3. **Neutro escuro** — dark mode limpo.
4. **Minimalista preto e branco** — alto contraste.
5. **Artesanal terroso** — tons naturais para makers.

Cada preset deve ser apenas ponto de partida. A empresa pode duplicar e editar.

### Validação do tema

Ao salvar, o sistema deve validar:

- JSON válido;
- campos mínimos presentes;
- cores em formato aceito (`#RRGGBB` no MVP);
- contraste mínimo razoável entre fundo/texto e primário/foreground;
- não permitir tema que deixe texto invisível;
- se inválido, exibir erros claros.

### Preview

A tela deve mostrar preview com:
...
- tabela;
- badge de status;
- alerta de sucesso/erro/aviso;
- exemplo de kanban;
- exemplo de formulário.

### Aplicação no app

O tema ativo deve ser carregado por empresa. O app deve:

- aplicar tokens no layout autenticado;
- usar fallback se não houver tema;
- não quebrar SSR/hidratação;
- suportar troca sem rebuild;
- manter tema por empresa, não por usuário, no MVP.
...
- Usuário owner consegue subir logo e vê-lo no shell do sistema.
- Usuário owner consegue escolher preset e aplicar no app.
- Usuário consegue editar cores via formulário simples.
- Usuário consegue editar JSON avançado e validar antes de salvar.
- Tema inválido não pode ser salvo.
- Se não houver configuração, app usa tema padrão.
- Componentes principais devem consumir tokens, não cores fixas.
- O sistema deve registrar auditoria ao alterar tema ou logo.

---

## 7.31 Kanban e fluxos configuráveis por empresa

### Objetivo

Permitir que cada empresa configure seus próprios fluxos operacionais. O sistema não deve assumir que toda produção possui etapa “Em cura”, pois outros contextos podem envolver costura, alimentos, cosméticos, papelaria, brownies, sabonetes, kits ou qualquer produto artesanal.

A Instante Âmbar pode usar etapas como “Em cura”, mas isso deve ser uma configuração de workflow, não uma regra fixa do código.
...
```txt
Nome visível da etapa ≠ comportamento técnico da etapa
```
...
A automação deve depender da chave técnica/comportamento, não do nome visível.

### Entidades com workflow configurável
...
Conjunto de etapas para uma entidade.

Exemplo:

```txt
Workflow: Produção de velas
Entidade: production
Etapas: Planejada → Separando materiais → Em produção → Em cura → Revisão → Liberada
```

#### Etapa

Coluna ou status dentro do workflow.
...
- se exige checklist;
- se exige motivo;
- se exige quantidade/perda;
- automação associada;
- permissões para mover de/para a etapa.

#### Automação

Comportamento interno opcional executado ao entrar ou sair da etapa.
...
- `owner`: gerencia tudo;
- `admin`: gerencia se permitido;
- `operator`: visualiza;
- outros: sem acesso ou somente leitura.
...
Interface com colunas arrastáveis.
...
- escolher cor;
- reordenar;
- definir se é inicial/final;
- configurar comportamento;
- configurar checklist;
- configurar regras de entrada/saída;
- arquivar etapa.

#### 3. Presets de workflow

Seedar presets iniciais:
...
- Etapa usada por registros históricos não deve ser deletada fisicamente; deve ser arquivada.
- Não permitir workflow ativo sem etapa inicial.
- Não permitir mais de uma etapa inicial por workflow.
- Não permitir automação destrutiva sem confirmação.
- Não permitir remover etapa que contém produções/pedidos abertos sem migrar registros.
- Ao editar workflow em uso, perguntar se alteração afeta apenas novos registros ou também registros abertos.
- Mudanças devem gerar auditoria.

### Associação de workflow

Produções devem receber workflow por uma destas regras:

1. workflow padrão da empresa;
2. workflow definido na categoria do produto;
3. workflow definido na receita/fórmula;
4. workflow escolhido manualmente ao planejar produção.

Ordem recomendada de precedência:

```txt
produção escolhida manualmente > receita > categoria do produto > padrão da empresa
```

### Comportamento no módulo de Produção

A tela de produção deve montar o kanban dinamicamente com base no workflow da produção.

Se houver produções com workflows diferentes, a tela deve permitir:

- filtrar por workflow;
- agrupar por workflow;
- mostrar alerta quando tentar visualizar colunas incompatíveis.

Para MVP, simplificação aceitável:

- empresa usa um workflow padrão de produção ativo;
- receitas podem sobrescrever workflow apenas em fase posterior.

### Checklists por etapa

Cada etapa pode ter checklist próprio.

Exemplo para “Em cura” no preset de velas:
...
- Owner consegue acessar “Fluxos e Kanban”.
- Owner consegue criar, duplicar e editar workflow.
- Owner consegue reordenar etapas do kanban.
- Owner consegue renomear “Em cura” para qualquer outro termo sem quebrar automações.
- Sistema consegue usar preset “Produção — Velas” para Instante Âmbar.
- Sistema consegue usar preset “Produção — Genérica” para outro tenant.
- Etapa com registros históricos não pode ser excluída fisicamente.
- Mudança de workflow registra auditoria.
- Produção deve renderizar colunas a partir do workflow configurado, não de lista hardcoded.
- O modo operação e as pick lists devem respeitar as etapas configuradas.
...
### 9.23 Branding white label

- `company_brand_settings`
- `brand_themes`
- `theme_change_history`

### 9.24 Workflows e kanban configurável
...
### 10.11 Branding white label

- Nenhuma cor crítica deve estar hardcoded em componentes operacionais.
- Tema inválido não pode ser aplicado.
- Sempre deve existir fallback visual seguro.
- Logo ausente não pode quebrar o layout; usar nome textual.
- Mudança de tema/logotipo deve gerar auditoria.

### 10.12 Kanban configurável

- Nomes visíveis das etapas nunca devem controlar lógica de negócio.
- Automação deve depender de `technical_key` ou `automation_type`.
- Etapas em uso não podem ser deletadas fisicamente.
- Todo workflow ativo deve ter uma etapa inicial.
- Registros históricos devem preservar a etapa usada na época, mesmo que o workflow seja alterado depois.
- O sistema deve permitir preset específico para velas, mas não pode depender dele para funcionar.
...
### 11.10 Ajuste v2.1 — Branding e workflows

Prioridade recomendada para incorporar os novos requisitos:

1. Criar tabelas base de branding (`company_brand_settings`, `brand_themes`).
2. Aplicar tokens visuais via CSS variables no app shell.
3. Criar página `/settings/branding` com upload de logo, preset e editor JSON simples.
4. Criar tabelas base de workflows (`workflows`, `workflow_steps`).
5. Seedar presets de produção e pedidos.
6. Substituir qualquer lista hardcoded de status por consulta ao workflow ativo.
7. Criar página `/settings/workflows` com CRUD inicial de etapas.
8. Integrar produção, pedidos, pick lists e modo operação ao workflow configurado.
9. Adicionar auditoria para alterações de branding e workflow.

Critério de ordem: primeiro fazer o app consumir tokens e workflows default; depois liberar edição avançada ao usuário.
...
### 12.9 Aceite adicional v2.1 — Branding white label

- Deve existir página de aparência da marca.
- Deve ser possível trocar logotipo da empresa.
- Deve ser possível escolher preset de cores.
- Deve ser possível editar tokens de tema via JSON.
- O app deve aplicar o tema sem alterar código.
- Deve existir preview antes de salvar.
- Deve haver validação básica de contraste/legibilidade.

### 12.10 Aceite adicional v2.1 — Kanban configurável

- Deve existir página de configuração de fluxos e kanban.
- Produção deve usar workflow configurado por empresa.
- Pedidos devem usar workflow configurado por empresa ou preset padrão.
- Deve ser possível renomear etapas sem quebrar regras internas.
- Deve ser possível reordenar colunas.
- Etapas protegidas não devem ser removidas se estiverem em uso.
- Modo Operação deve respeitar as etapas configuradas.
