...
- [x] **Public order tracking API (delivered foundation):** CORS-enabled `GET /api/public/track` (token link or company-scoped order# + email/CEP) returns a sanitized payment + fulfillment + carrier view for the separate site. It is source-agnostic, so the items below must feed the order's `status` / `paymentStatus` / `tracking` fields rather than a separate tracking store.
...
- [x] **Public tracking follow-ups:** rate-limiting/abuse protection and company scoping for order# + email/CEP lookup are delivered. Optional richer status history can wait until per-event marketplace/carrier timestamps are needed.
