/* ============================================================
   ui.jsx — shadcn-style primitives + lucide-style icons
...
/* ---------------- Roles (multi-tenant) ---------------------------- */
const ROLE_LABELS = { owner: 'Proprietária', admin: 'Administrador(a)', operator: 'Operador(a)' };
const ROLE_META = {
...
/* ---------------- Button ------------------------------------------ */
function Button({ variant = 'default', size = 'md', className, children, icon, iconRight, ...rest }) {
  const base = 'om-btn';
  return (
    <button className={cn(base, `om-btn--${variant}`, `om-btn--${size}`, className)} {...rest}>
...
function CardHeader({ className, children }) { return <div className={cn('om-card-head', className)}>{children}</div>; }
function CardTitle({ className, children }) { return <div className={cn('om-card-title', className)}>{children}</div>; }
function CardContent({ className, children, style }) { return <div className={cn('om-card-body', className)} style={style}>{children}</div>; }

/* ---------------- Badge / Status ---------------------------------- */
function Badge({ tone = 'neutral', children, dot, className }) {
  return (
...
function Textarea({ className, ...rest }) { return <textarea className={cn('om-input om-textarea', className)} {...rest} />; }
...
/* ---------------- Progress ---------------------------------------- */
function Progress({ value, tone = 'primary', className }) {
  return (
    <div className={cn('om-progress', className)}>
      <div className={cn('om-progress-fill', `om-progress-fill--${tone}`)} style={{ width: `${Math.max(0, Math.min(100, value))}%` }} />
    </div>
  );
}
...
/* ---------------- Separator / Empty ------------------------------- */
function Sep({ className }) { return <div className={cn('om-sep', className)} />; }
function Empty({ icon = 'inbox', title, hint }) {
  return (
...
// persist view preference per screen
function useView(screenKey, fallback = 'list') {
  const k = 'atelie-view-' + screenKey;
  const [v, setV] = React.useState(() => localStorage.getItem(k) || fallback);
  const set = (nv) => { setV(nv); localStorage.setItem(k, nv); };
  return [v, set];
}

/* ---------------- Modal (centered form dialog) -------------------- */
function Modal({ open, onClose, title, subtitle, icon, footer, children, width = 580 }) {
  React.useEffect(() => {
    if (!open) return;
...
function Field({ label, children, hint, required, error, className, style }) {
  return (
...
function Select({ value, onChange, options, placeholder, className, ...rest }) {
  return (
...
/* ---------------- Sorting ----------------------------------------- */
function useSort(rows, accessors, initialKey = null, initialDir = 'desc') {
...
function SortTh({ label, k, sort, align, style }) {
...
Object.assign(window, {
  cn, Icon, Button, Card, CardHeader, CardTitle, CardContent,
  Badge, Input, Textarea, Tabs, Progress, Avatar, Sep, Empty, Stat,
  ViewToggle, useView, Modal, Field, Select, Stepper, useSort, SortTh, toast,
  ROLE_LABELS, ROLE_META,
});
