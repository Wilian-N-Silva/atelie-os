/* ============================================================
   command.jsx — ⌘K command palette + Notifications center
   ============================================================ */

/* ---------------- Command Palette --------------------------------- */
function CommandPalette({ open, onClose, go }) {
  const { items, orders, production, recipes, ORDER_STATUS } = window.DB;
  ...
  const itemHits = ql ? items.filter(i => match(i.name) || match(i.sku) || match(i.code) || match(i.variant)).slice(0, 6).map(i => ({
  ...
  const orderHits = ql ? orders.filter(o => match(o.num) || match(o.customerName) || match(o.code)).slice(0, 5).map(o => ({
  ...
  const prodHits = ql ? production.filter(p => match(p.num) || match(p.productName) || match(p.code)).slice(0, 4).map(p => ({
    id: 'p-' + p.id, kind: 'prod', icon: 'producao', title: `${p.num} · ${p.productName}`,
    sub: window.DB.PROD_STATUS[p.status].label, meta: p.code, go: { screen: 'producao' },
  })) : [];
  const recipeHits = ql ? recipes.filter(r => match(r.name) || match(r.productName)).slice(0, 3).map(r => ({
  ...
  const choose = (it) => { if (!it) return; onClose(); go(it.go.screen, it.go); };
...
function NotifCenter({ notifications, unread, markRead, markResolved, markAllRead, go, onClose }) {
  ...
  const handleAction = (n) => { markRead(n.id); onClose(); go(n.action.screen, n.action); };
  ...
  return (
    <div className="notif-backdrop" onClick={onClose}>
      ...
      <div className="notif-tabs">
        {[['todas', 'Todas'], ['nao-lidas', 'Não lidas'], ['resolvidas', 'Resolvidas']].map(([v, l]) => (
...
Object.assign(window, { CommandPalette, NotifCenter });
