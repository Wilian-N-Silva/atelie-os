/* ============================================================
   onboarding.jsx — account setup after self-serve signup.
   Create ateliê → details → (optional) invite team → enter app.
   Exposes window.Onboarding. onDone({ companyName }) → Root.
   ============================================================ */
...
function Onboarding({ user, onDone }) {
  ...
  const setInvite = (i, patch) => setInvites(list => list.map((x, idx) => idx === i ? { ...x, ...patch } : x));
  const addInvite = () => setInvites(list => [...list, { email: '', role: 'operator' }]);
  const rmInvite = (i) => setInvites(list => list.length === 1 ? [{ email: '', role: 'operator' }] : list.filter((_, idx) => idx !== i));

  const finish = () => {
    if (logo) { try { localStorage.setItem('atelie-logo', logo); } catch (e) {} }
    if (validInvites.length) {
      try { localStorage.setItem('atelie-pending-invites', JSON.stringify(validInvites)); } catch (e) {}
    }
    onDone({ companyName: coName.trim() });
  };
        ...
        <div className="ob-panel">
          ...
          {step === 2 && (
            <div>
              ...
              <div className="ob-foot">
                <Button variant="default" size="lg" onClick={finish} iconRight="arrowRight">Entrar no Ateliê OS</Button>
              </div>
            </div>
          )}
        </div>
